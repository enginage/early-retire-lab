...
let isLoadingDomesticEtfCache = false;
let domesticEtfCachePromise = null;

function DomesticETFSelector({ isOpen, onClose, onSelect }) {
  ...
  const loadETFs = async () => {
    // 캐시가 이미 있으면 캐시 사용
    if (domesticEtfCache) {
      setEtfs(domesticEtfCache);
      return;
    }

    // 이미 로딩 중이면 기존 Promise 대기
    if (isLoadingDomesticEtfCache && domesticEtfCachePromise) {
      try {
        const data = await domesticEtfCachePromise;
        setEtfs(data);
      } catch (err) {
        setError('국내ETF 목록을 불러오는데 실패했습니다.');
      }
      return;
    }
    ...
    try {
      ...
      domesticEtfCachePromise = fetch(`${API_BASE_URL}?skip=0&limit=10000`, {
        ...
        if (response.ok) {
          ...
          domesticEtfCache = data; // 캐시에 저장
          isLoadingDomesticEtfCache = false;
          domesticEtfCachePromise = null;
          return data;
        } else {
          isLoadingDomesticEtfCache = false;
          domesticEtfCachePromise = null;
          throw new Error('국내ETF 목록을 불러오는데 실패했습니다.');
        }
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm">
      <div className="bg-wealth-card rounded-xl border border-gray-800 shadow-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden flex flex-col">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-800">
          <h2 className="text-2xl font-bold text-white">국내ETF 선택</h2>
          <button
            onClick={onClose}
...
export const findDomesticETFByCode = (code) => {
  if (!domesticEtfCache) return null;
  return domesticEtfCache.find(etf => etf.ticker === code || etf.ticker.toLowerCase() === code.toLowerCase());
};

// 캐시를 초기화하는 함수 export (필요시 사용)
export const ensureDomesticETFCache = async () => {
  if (domesticEtfCache) {
    return domesticEtfCache;
  }

  if (isLoadingDomesticEtfCache && domesticEtfCachePromise) {
    return await domesticEtfCachePromise;
  }
...
export default DomesticETFSelector;
