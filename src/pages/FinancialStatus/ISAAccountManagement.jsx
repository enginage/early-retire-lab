...
import ISAAccountDetail from './ISAAccountDetail';
...
const DETAIL_API_BASE_URL = 'http://localhost:8000/api/v1/isa-account-details';
...
function ISAAccountManagement() {
  ...
  const [nonTaxTypeMap, setNonTaxTypeMap] = useState({}); // 비과세유형코드명 매핑
  ...
  const hasRegistAccount = () => {
    return accounts.some(account => account.account_status_code === 'regist');
  };

  const handleAdd = () => {
    if (hasRegistAccount()) {
      setError('계좌상태가 등록 상태인 계좌가 이미 존재합니다. 등록 중인 계좌는 하나만 존재할 수 있습니다.');
      return;
    }
    setNewRow({
      financial_institution_code: '',
  ...
  const handleSave = async (accountData) => {
    try {
      ...
      if (!accountData.non_tax_type || accountData.non_tax_type.trim() === '') {
        setError('비과세유형을 선택해주세요.');
        setLoading(false);
        return;
      }
      ...
      const payload = {
        ...
        non_tax_type: accountData.non_tax_type,
      };
  ...
  return (
    <div className="space-y-8">
      <div>
        <h1 className="text-3xl font-bold text-white mb-2">ISA 관리</h1>
        <p className="text-wealth-muted">ISA 계좌 정보를 관리하고 종목별 상세 정보를 확인하세요.</p>
      </div>
      ...
      <div className="bg-wealth-card/50 backdrop-blur-sm rounded-xl border border-gray-800 shadow-xl p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={handleAdd}
            disabled={loading || editingId === 'new' || hasRegistAccount()}
            ...
            title={hasRegistAccount() ? '계좌상태가 등록 상태인 계좌가 이미 존재합니다. ' : ''}
          >
            + 추가
          </button>
        </div>
          ...
          <DataGrid
            ...
            renderNewRow={() => (
              <>
                ...
                <td className="py-3 px-4">
                  <CommonCodeSelector
                    masterCode="isa_non_tax_type"
                    value={newRow.non_tax_type}
                    onChange={(e) => setNewRow({ ...newRow, non_tax_type: e.target.value })}
                    placeholder="비과세유형 선택"
                  />
                </td>
                <td className="py-3 px-4">
                  <CommonCodeSelector
            ...
            renderEditRow={(row) => (
              <>
                ...
                <td className="py-3 px-4">
                  <CommonCodeSelector
                    masterCode="isa_non_tax_type"
                    value={row.non_tax_type}
                    onChange={(e) => handleInputChange(row.id, 'non_tax_type', e.target.value)}
                    placeholder="비과세유형 선택"
                  />
                </td>
                <td className="py-3 px-4">
        ...
        {selectedAccountId && (
          <div className="mt-8">
            <ISAAccountDetail accountId={selectedAccountId} />
          </div>
        )}
      </div>
...
export default ISAAccountManagement;
